 ...
 * Attention		 : None
 *******************************************************************************/
void PutChar(uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor, uint8_t is_reverse)
{
	uint16_t i, j;
	...
	for (i = 0; i < 16; i++)
	{
		...
		for (j = 0; j < 8; j++)
		{
			...
			{
				LCD_SetPoint(Xpos + j * (direction), Ypos + i * (direction), charColor); /* �ַ���ɫ */
			}
			else
			{
				LCD_SetPoint(Xpos + j * (direction), Ypos + i * (direction), bkColor); /* ������ɫ */
			}
		}
	}
}
 ...
 * Attention		 : None
 *******************************************************************************/
void GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str, uint16_t Color, uint16_t bkColor, uint8_t is_reverse)
{
	uint8_t TempChar;
	...
	// last string memory address
	end_condition = str + len;
	do
	{
		TempChar = *str++;
		PutChar(Xpos, Ypos, TempChar, Color, bkColor, is_reverse);
		if (is_reverse)
		{
			if (Xpos > 8)
			{
